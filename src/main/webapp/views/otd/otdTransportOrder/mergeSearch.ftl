...
<script>
    $(function () {
        var GridModel = function () {
            ...
            self.dataSource = kendo.utils.createDataSource("transportOrderId", "/otdTransportOrderView/getMergeSearchDataSource/${clientId}");
            ...
            self.selectAll= function(){
                ...
                for(var i = 1;i<=self.dataSource.data().length+1;i++){
                    select.push("#transportSearch tr:eq("+i+")");
                }
                console.info(select.toString());
                var grid11 = $("#transportSearch").data("kendoGrid");
                grid11.select(select.toString());
                select.length=0;
            }
