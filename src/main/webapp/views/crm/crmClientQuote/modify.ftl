...
<div>
    <form id="quote-modify-form" method="post" data-bind="submit:save" class="form-horizontal">
...
<script>
    function ModifyQuoteModel() {
        var self = this;
        self.model = {};
        self.model = ko.mapping.fromJS(${crmClientQuoteJson});
        ...
        self.save = function () {
            // 检查表单是否通过验证
            if (!$("#quote-modify-form").validate().valid()) return;

            // 发送AJAX请求保存数据
            $.ajax({
                type: "POST",
                contentType: "application/json",
                url: "/crmClientQuote/updateCrmClientQuote",
    ...
    $(function () {
        $("#quote-modify-form").validate();
        var viewQuoteModel = new ModifyQuoteModel();
        ko.applyBindings(viewQuoteModel, document.getElementById("quote-modify-form"));
    });
