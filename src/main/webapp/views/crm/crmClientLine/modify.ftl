...
<div>
    <form id="crmclientLine-modify-form" method="post" data-bind="submit:save" class="form-horizontal">
...
<script>
    function crmclientLineModel() {
        var self = this;
        self.model = ko.mapping.fromJS(${UUIDJson});
        self.transportType = commonData.transportType;
        self.save = function () {
            // 检查表单是否通过验证
            if (!$("#crmclientLine-modify-form").validate().valid()) return;
            $.ajax({
                type: "POST",
                contentType: "application/json",
                url: "/crmClientLine/update",
    ...
    $(function () {
        $("#crmclientLine-modify-form").validate();
        var viewModel = new crmclientLineModel();
        ko.applyBindings(viewModel, document.getElementById("crmclientLine-modify-form"));
    });
</script>